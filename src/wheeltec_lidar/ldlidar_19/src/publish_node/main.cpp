...
int main(int argc, char **argv) {
  ...
  nh_private.param("flag_parted", setting.flag_parted, bool(false));
  ...
  ROS_INFO("[ldrobot] <laser_scan_dir>: %s,<flag_parted>: %s,<angle_crop_min>: %f,<angle_crop_max>: %f",
   (setting.laser_scan_dir?"Counterclockwise":"Clockwise"), (setting.flag_parted?"true":"false"), setting.angle_crop_min, setting.angle_crop_max);
...
void  ToLaserscanMessagePublish(Points2D& src, LiPkg* commpkg, LaserScanSetting& setting, ros::Publisher& lidarpub) {
  ...
  if (commpkg->GetSpeedOrigin() > 0) {
    ...
    output.angle_min = angle_min-3.1415926;
    output.angle_max = angle_max-3.1415926;
    ...
    for (auto point : src) {
      ...
      if(setting.flag_parted){// Angle crop setting, Mask data within the set angle range
                ...
                if(disable_angle_min_range[j]>disable_angle_max_range[j])
                 {
                  if( (dir_angle<(disable_angle_max_range[j])) || (dir_angle>=(disable_angle_min_range[j])) ){
                    range =std::numeric_limits<float>::quiet_NaN();  // 如果不在角度不在扫描范围内，将该角度对应的目标距离置为无穷大
                    intensity = 0;
                    }
                  }
                else{
     ...
     if (range <range_min  ||  range>range_max)
      ...
      float angle = ANGLE_TO_RADIAN(dir_angle)-3.1415926; // Lidar angle unit form degree transform to radian
